...
namespace BaeCoach.Controllers
{
    public class ResponsesController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "ResponseID,RespronsText,FK_CoachID,FK_ResponseID")] Response response)
        {

            if (ModelState.IsValid)
            {
                response.FK_PostID = Convert.ToInt32(Session["PostID"]);
                response.Topic = Convert.ToInt32(Session["Topic"]);
